...
int main(int argc, char* argv[]) {
    ...
    int out_fd = wrapped_open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, &perm); 
    
    uint8_t buff;
    while (wrapped_read(in_fd, &buff, sizeof(buff)) > 0) {
        ...
        const uint8_t* byte_arr = (uint8_t*) &result;
        write(out_fd, byte_arr + 1, sizeof(uint8_t));
        write(out_fd, byte_arr, sizeof(uint8_t));
    }
